...
func main() {
	...
	go func() {
		var err error
		server, err = proxy.NewServer(&cfg.Proxy.Bind, &cfg.Proxy.Motd, &cfg.Proxy.MaxPlayers, &cfg.Proxy.SyncMaxPlayers, &cfg.Proxy.Authenticate, &cfg.Proxy.FallbackRegexMatch, cfg, cfg, proxyConnect)
		if err != nil {
			serverErr <- err
			return
		}
